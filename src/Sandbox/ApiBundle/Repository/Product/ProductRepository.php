...
class ProductRepository extends EntityRepository
{
    public function getProductsForClient($roomType, $buildingId, $startTime, $timeUnit, $endTime, $allowedPeople)
    {
        $query = $this->createQueryBuilder('p')
            ...
            ->leftjoin('SandboxApiBundle:Product\ProductAttachment', 'pa', 'WITH', 'pa.productId = p.id')
            ->leftJoin('SandboxApiBundle:Room\RoomRentedDate', 'rd', 'WITH', 'rd.roomId = p.roomId')
            ->leftJoin('SandboxApiBundle:Location\City', 'c', 'WITH', 'c.id = r.city')
            ->leftJoin('SandboxApiBundle:Location\Building', 'b', 'WITH', 'b.id = r.building')
            ->leftJoin('SandboxApiBundle:Location\Floor', 'f', 'WITH', 'f.id = r.floor')
