 ...
 */
class Version920170427090821 extends AbstractMigration implements ContainerAwareInterface
{
    ...
    public function postUp(Schema $schema)
    {
        ...
        $version25x = $em->getRepository('SandboxApiBundle:Menu\Menu')
            ->findBy(array(
                'minVersion' => '2.5.0',
            ));

        $version24x = $em->getRepository('SandboxApiBundle:Menu\Menu')
            ->findBy(array(
                'minVersion' => '2.4.0',
            ));

        $version23x = $em->getRepository('SandboxApiBundle:Menu\Menu')
            ->findBy(array(
                'minVersion' => '2.3.10',
            ));
        ...
        foreach ($version25x as $version1) {
            $version1->setMainJson($mainJson);
            $version1->setProfileJson($profileJson);
            $version1->setHomeJson($homeJson);
        }

        foreach ($version24x as $version2) {
            $version2->setMainJson($mainJson);
            $version2->setProfileJson($profileJson);
            $version2->setHomeJson($homeJson);
        }

        foreach ($version23x as $version3) {
            $version3->setMainJson($mainJson);
            $version3->setProfileJson($profileJson);
            $version3->setHomeJson($homeJson);
        }
